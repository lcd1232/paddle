...
func TestInvoiceSentWebhook(t *testing.T) {
	type args struct {
		form url.Values
	}
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				form: url.Values{
					...
					"invoiced_at":                    {"2020-10-30 00:00:00"},
					"currency":                       {"USD"},
					"product_id":                     {"1234"},
					"product_name":                   {"name"},
			...
			want: InvoiceSent{
				...
				InvoicedAt:                   time.Date(2020, 10, 30, 0, 0, 0, 0, time.UTC),
				Currency:                     "USD",
				ProductID:                    "1234",
				ProductName:                  "name",
				...
				Passthrough:                  "some data",
				DateCreated:                  time.Date(2020, 10, 30, 0, 0, 0, 0, time.UTC),
				BalanceCurrency:              "USD",
				PaymentTax:                   "1234",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := c.ParseInvoiceSentWebhook(tt.args.form)
			if tt.wantErr {
				require.Error(t, err)
				return
			}
