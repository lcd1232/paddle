...
func TestInvoicePaidWebhook(t *testing.T) {
	type args struct {
		form url.Values
	}
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				form: url.Values{
					...
					"fee":                            {"1234"},
					"earnings":                       {"1234"},
					"balance_earnings":               {"1234"},
					"balance_fee":                    {"1234"},
					"balance_tax":                    {"1234"},
					"balance_gross":                  {"1234"},
					"date_reconciled":                {"2020-10-30"},
					"p_signature":                    {"signature"},
					"event_time":                     {"2020-10-30 00:00:00"},
				},
			},
			want: InvoicePaid{
				AlertName:                    AlertInvoicePaid,
				AlertID:                      "27120763",
				PaymentID:                    "1234",
				Amount:                       "9.99",
				SaleGross:                    "0.10",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := c.ParseInvoicePaidWebhook(tt.args.form)
			if tt.wantErr {
				require.Error(t, err)
				return
			}
